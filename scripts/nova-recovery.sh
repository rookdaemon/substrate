#!/bin/bash
# Nova substrate recovery script for systemd OnFailure handler
# This script attempts to diagnose and fix nova-substrate crashes using Claude CLI

set -euo pipefail

# Configuration
STATE_DIR="/var/lib/nova-substrate"
ATTEMPT_FILE="$STATE_DIR/recovery-attempts"
MAX_ATTEMPTS=3
CLAUDE_TIMEOUT=300  # 5 minutes in seconds
RECIPIENT_EMAIL="lbsa71@hotmail.com"
SUBSTRATE_HOME="/home/nova/substrate"
SERVICE_NAME="nova-substrate.service"
LOG_TAG="nova-substrate-recovery"
...
# Main recovery logic
main() {
    log_info "Nova substrate recovery service triggered"
    ...
    if [ "$attempt_count" -ge "$MAX_ATTEMPTS" ]; then
        ...
        local log_file="/tmp/nova-substrate-recovery-final-logs.txt"
        journalctl -u "$SERVICE_NAME" --no-pager -n 100 > "$log_file" 2>&1 || true

        send_email \
            "Nova Substrate Recovery Failed - Manual Intervention Required" \
            "The nova-substrate service has failed $MAX_ATTEMPTS times and automatic recovery has been exhausted.
...
" \
    ...
    local rebuild_output="/tmp/nova-substrate-recovery-rebuild-output.txt"
    ...
    if [ "$rebuild_success" = true ]; then
        ...
        if restart_substrate; then
            log_info "Nova substrate service restarted successfully after rebuild"

            send_email \
                "Nova Substrate Recovery Successful via Rebuild (Attempt $attempt_count)" \
                "The nova-substrate service was recovered by a standard rebuild (nvm use --lts, git pull, npm install --include=dev, npm run build).
...
"
    ...
    local claude_prompt="The nova-substrate process has crashed and the recovery service isn't resurrecting it. Please fix the root cause, build, lint, test, push and restart services."

    local claude_output="/tmp/nova-substrate-recovery-claude-output.txt"
    ...
    if timeout "$CLAUDE_TIMEOUT" claude -p "$claude_prompt" --dangerously-skip-permissions > "$claude_output" 2>&1; then
        ...
        if grep -qiE "success|fixed|resolved" "$claude_output"; then
            log_info "Claude reported successful fix. Restarting nova-substrate service..."

            # Restart the service
            if restart_substrate; then
                log_info "Nova substrate service restarted successfully"

                # Send success notification
                send_email \
                    "Nova Substrate Recovery Successful via Claude (Attempt $attempt_count)" \
                    "The nova-substrate service has been successfully recovered by the Claude-assisted recovery system.
...
"
                ...
                log_error "Failed to restart nova-substrate service after Claude fix"
                claude_exit_code=1
            fi
        else
    ...
    send_email \
        "Nova Substrate Recovery Attempt $attempt_count Failed" \
